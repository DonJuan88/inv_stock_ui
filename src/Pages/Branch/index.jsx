...
export default function BranchIndex() {
  ...
  const [formData, setFormData] = useState({ code: "", Branch: "" }); // Stores the input value for updating
  ...
  const validate = () => {
    ...
    // Username validation: must not be empty and at least 3 characters
    if (!formData.code) {
      newErrors.code = "Code is required";
      isValid = false;
    } else if (formData.code.length < 3) {
      newErrors.username = "Code must be at least 3 characters";
      isValid = false;
    }

    if (!formData.Branch) {
      newErrors.Branchs = "Branch is required";
      isValid = false;
    } else if (formData.Branch.length < 3) {
      newErrors.email = "Branch must be at least 3 characters";
      isValid = false;
    }
  ...
  const handleUpdate = async () => {
    if (validate()) {
      ...
      try {
        await updateBranch(selectedId, formData);
        setFormData({ code: "", Branch: "" });
        useEffect;
        setSelectedId(null);
      } catch (error) {
  ...
  const handleEditClick = (item) => {
    ...
    console.log(item.ID);
    setFormData({ code: item.code, Branch: item.Branchs });
  };

  const deleteTheBranch = (id) => {
    try {
      deleteBranch(id);
      setBranch(Branch.filter((data) => data.ID !== id));
      useEffect;
    } catch (error) {
      console.log(error);
  ...
  const getAllBranchs = async () => {
    try {
      const data = await getBranchs(); // Call the refactored API service
      setBranch(data); // Update the state with fetched data
    } catch (err) {
  ...
  return (
    <div className="mx-10 mt-5 mb-7 min-h-screen backdrop-blur-lg bg-opacity-80 rounded-lg shadow-lg p-5 bg-gray-300">
      <div className="col-span-2 p-4 ">
        <div className="col-span-1 p-4">
          <div className="mx-auto backdrop-blur-lg bg-opacity-80 rounded-lg shadow-lg p-5 bg-gray-300 text-black">
            <form>
              <div className="grid grid-cols-4 gap-3 w-4/5">
                <div className="text-center font-bold my-1">New Input :</div>
                <div>
                  <input
                    type="text"
                    name="code"
                ...
                <div>
                  ...
                  />
                  {/* 
                {error.Branch && <p className="text-red-500">{error.Branch}</p>} */}
                </div>{" "}
        ...
        <h2 className="font-bold mb-2 text-center">Branch List</h2>
        <div className="flex flex-col  h-screen">
          <div className=" flex-grow overflow-auto">
            <table className="table-fixed relative w-full overflow-auto max-h-fit  grid-cols-1 border border-spacing-1 border-collapse rounded-lg scroll-pl-6 snap-y">
              ...
              <tbody className="border-collapse border-spacing-2 align-middle overflow-y-auto ">
                    ...
                    <td className=" p-2 ">
                      <div className="flex items-center gap-3">
                        <button
                          ...
                          onClick={() => handleEditClick(Branchs)}
                        >
                          <svg
                        ...
                        <button
                          onClick={() => deleteTheBranch(Branchs.ID)}
