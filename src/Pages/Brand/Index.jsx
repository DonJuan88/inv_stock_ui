...
function Brand() {
  ...
  const validate = () => {
    ...
    if (!formData.brand) {
      newErrors.brands = "Brand is required";
      isValid = false;
    } else if (formData.brand.length < 3) {
      newErrors.email = "Brand must be at least 3 characters";
      isValid = false;
    }
  ...
  const handleEditClick = (item) => {
    ...
    console.log(item.ID);
    setFormData({ code: item.code, brand: item.brands });
  };

  const deleteTheBrand = (id) => {
    try {
      deleteBrand(id);
                  ...
                  >
                    ...
                    <td className=" p-2 ">
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          onClick={() => deleteTheBrand(brands.ID)}
