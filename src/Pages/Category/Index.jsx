...
const CategoryIndex = () => {
  ...
  const [formData, setFormData] = useState({ code: "", category: "" }); // Stores the input value for updating
  ...
  const validate = () => {
    ...
    if (!formData.category) {
      newErrors.categorys = "category is required";
      isValid = false;
    } else if (formData.category.length < 3) {
      newErrors.email = "category must be at least 3 characters";
      isValid = false;
    }
  ...
  const handleEditClick = (item) => {
    ...
    console.log(item.ID);
    setFormData({ code: item.code, category: item.categories });
  };
  ...
  return (
    <div>
      <div className="mx-10 mt-5 mb-7 backdrop-blur-lg bg-opacity-80 rounded-lg shadow-lg p-5 bg-gray-300">
        ...
        <h2 className="font-bold mb-2 text-center">Category List</h2>

        <div className="overflow-auto">
          <table className="table-fixed w-full overflow-auto max-h-fit h-3/5 grid-cols-1 border border-spacing-1 border-collapse rounded-lg scroll-pl-6 snap-y">
            <thead className=" sticky bg-slate-400 text-center">
              <tr>
                <th className="">#</th>
            ...
            <tbody className="border-collapse border-spacing-2 align-middle overflow-y-auto text-left ">
